...
void runBenchmark(const std::string &json, BenchmarkRun &benchmark)
{
  ...
  fprintf(stderr, "Execution of %s: size is %zu and it took %f. Name is %s\n", benchmark.name().c_str(),
          benchmark.max_size, ms.count(), benchmark.second_name.c_str());
}
...
struct SaJsonRun : public BenchmarkRun
{
  ...
  void run(const std::string &json) override
  {
    const sajson::document &document =
      sajson::parse(sajson::dynamic_allocation(), sajson::string(json.c_str(), json.size()));
